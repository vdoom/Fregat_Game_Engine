	...
	namespace System
	{
		...
		MaterialBase* AssetsManager::GetMaterial(const std::string& t_name, Shader* t_shader, TextureBase* t_texture)
		{
			...
			else
			{
				...
				m_materials[t_name] = materialTest;// std::move(std::unique_ptr<MaterialBase>(materialTest));
				return materialTest;
			}
			return nullptr;
		}
		...
		{
			std::unique_ptr<Fregat::Render::Shader> shaderTest = std::move(Fregat::Render::Shader::CreateShader("simple_with_light.vert", "simple_with_light.frag"));
