...
namespace Fregat
{
	namespace Render
	{
		...
		Math::Vec3 Camera::GetPosition() { return GetPosFromMat4(m_entity.GetTransform().GetGlobalTransformMatrix()); };//m_parent.GetTransform().GetGlobalTransformMatrix() * m_position; }
		
		Math::Mat4 Camera::GetMatrix()
		{
			return GetEntity()->GetTransform().GetGlobalTransformMatrix();
		}
	}
}
