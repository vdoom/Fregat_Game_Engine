...
namespace Fregat
{
	namespace Render
	{
		...
		void MeshPlane::InitMesh(Math::Rect t_size)
		{
			m_rect = t_size;
			...
			m_indices[5] = 2;
			BakeMesh();
			m_aabb = Math::AABB(Math::Vec3(m_rect.GetBottomLeft().x, m_rect.GetBottomLeft().y, 0),
								Math::Vec3(m_rect.GetTopRight().x, m_rect.GetTopRight().y, 0));
			//m_AA = Math::Vec3(m_rect.GetBottomLeft().x, m_rect.GetBottomLeft().y, 0);
			//m_BB = Math::Vec3(m_rect.GetTopRight().x, m_rect.GetTopRight().y, 0);
			//m_centr = m_BB - m_AA;
		}
